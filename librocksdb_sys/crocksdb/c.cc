...
struct crocksdb_timestamp_comparator_t : public Comparator {
  ...
  explicit crocksdb_timestamp_comparator_t(size_t timestamp_size) : Comparator(timestamp_size) {}
  virtual int Compare(const Slice& a, const Slice& b) const override {
    return a.compare(b);
  }

  virtual int CompareWithoutTimestamp(const Slice& a, const Slice& b) const override {
    assert(a.data_ != nullptr && b.data_ != nullptr);
    const size_t min_len = (a.size_ < b.size_) ? a.size_ : b.size_;
    assert(min_len >= timestamp_size());
    int r = memcmp(a.data_, b.data_, min_len - timestamp_size());
    if (r == 0) {
      if (a.size_ < b.size_)
        r = -1;
...
//                                     const Slice&) const override {}
  ...
  virtual bool IsSameLengthImmediateSuccessor(
                const Slice& s, const Slice& t) const override {
...
void crocksdb_options_set_user_comparator(
    crocksdb_options_t* opt,
    size_t timestamp_size) {
  opt->rep.comparator = new crocksdb_timestamp_comparator_t(timestamp_size);
}
...
void crocksdb_readoptions_set_user_timestamp(
    crocksdb_readoptions_t* opt,
    const char* key, size_t keylen) {
    if (key != nullptr) {
        opt->timestamp = Slice(key, keylen);
